    ...
    let response_items: Vec<BridgeTransactionsResponse> = joined_data
        ...
        .map(|(source_tx, dest_tx)| {
            ...
            BridgeTransactionsResponse {
                ...
                slot_number: source_tx.source_height,
                l2_slot_number: dest_tx.destination_height,
                block_number: source_tx.source_height,
...
pub async fn get_l2_withdraws(
    ...
    let response_items: Vec<BridgeTransactionsResponse> = joined_data
        ...
        .map(|(source_tx, dest_tx)| {
            ...
            BridgeTransactionsResponse {
                l1_tx_hash: dest_tx.destination_tx_hash.clone(),
                l2_tx_hash: source_tx.source_tx_hash.clone(),
                slot_number: dest_tx.destination_height,
                l2_slot_number: source_tx.source_height,
                block_number: dest_tx.destination_height,
                status: dest_tx.destination_status_code,
                nonce: source_tx.source_nonce,
                chain_id: source_tx.source_chain_id,
                l1_token: source_tx.destination_token_address.clone(),
                l2_token: source_tx.source_token_address.clone(),
                from: source_tx.source_from_address.clone(),
                to_twine_address: source_tx.target_recipient_address.clone(),
                amount: source_tx.amount.clone(),
                created_at,
